...
describe('Subscribable helper', function () {
    ...
    it('should call all subscribers with the scope.lastestValue on calling "valueHasMutated"', function () {
        subscribableHelperModule(fakeYal);

        var toExtend = {};
        var scope = {
            lastestValue: Math.random().toString(26)
        };
        var subscriber = chai.spy();
        ...
        expect(subscriber).to.have.been.called.with(scope.lastestValue);
    });
